...
lm = torch.load('./models/RRDB_BlurIso.pth')
...
with torch.no_grad():
    ...
    for testset in ['Set5', 'Set14', 'BSDS100', 'Urban100', 'Manga109']:
        ...
        if not isdir('./output/Iso/{}'.format(testset)):
            mkdir('./output/Iso/{}'.format(testset))

        psnrs = []
        ssims = []
        lpips = []

        files_gt = glob.glob('./Gaussian8/gt/{}/*.png'.format(testset))
        files_gt.sort()
        ...
        for i in tqdm(range(len(files_gt))):
            ...
            gt_fn = files_gt[i].split("/")[-1][:-4]

            for s in range(18, 33, 2):
                sigma = s / 10.0

                # Load images
                img = _load_img_array('./Gaussian8/lr_x4/{}/{}_sigma{:.1f}.png'.format(testset, gt_fn, sigma))
                ...
                # Save to file
                out_fn = '{}_sigma{:.1f}.png'.format(gt_fn, sigma)
                Image.fromarray(batch_out).save('./output/Iso/{}/{}.png'.format(testset, out_fn))
